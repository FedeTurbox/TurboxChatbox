...
class AIResponder:
    ...
    def answer_question(self, question, top_k=5, score_threshold=0.30, title_boost=0.25):
        ...
        para las coincidencias de título y devuelve información de depuración.
        """
        if self.corpus_embeddings is None or not self.corpus_chunks:
            return None, []
        ...
        for hit in hits:
            ...
            if normalized_question_words.issubset(chunk_title_words):
                ...
                modifier = f"Title Boost (+{title_boost:.2f})"
        ...
        # Prepara la información de depuración con los 2 mejores resultados
        debug_info = []
        for hit in sorted_hits[:2]:
            debug_info.append({
                "match": self.corpus_chunks[hit['corpus_id']],
        ...
        if best_hit['score'] > score_threshold:
            ...
            return self.corpus_chunks[best_hit['corpus_id']], debug_info
...
class ValidadorConsultas:
    ...
    def buscar_coincidencia_reglas(self, consulta):
        """
        Busca coincidencias y devuelve las 2 mejores junto con información de depuración.
        """
        ...
        # Ordenar todas las coincidencias por puntaje
        coincidencias_ordenadas = sorted(todas_las_coincidencias, key=lambda x: x['score'], reverse=True)
        
        # Preparar la información de depuración con los 2 mejores
        debug_info = []
        for match in coincidencias_ordenadas[:2]:
            qid_match = re.search(r"\((Q\d+)\)", match['archivo'])
            qid = qid_match.group(1) if qid_match else "N/A"
            debug_info.append({
                "match": match['variante'],
                "score": f"{match['score']:.4f}",
                "modifier": "SequenceMatcher",
                "qid": qid 
            })

        mejor_coincidencia = coincidencias_ordenadas[0] if coincidencias_ordenadas else None
        
        if mejor_coincidencia and mejor_coincidencia["score"] > 0.65:
            return mejor_coincidencia, debug_info
        else:
            return None, debug_info
    ...
    def procesar_consulta(self, consulta):
        """
        Procesa la consulta y devuelve la respuesta junto con un diccionario de depuración.
        """
        debug_info = {
            "query": consulta,
            "rules": [],
        ...
        if not os.path.isdir(self.instance_path):
            ...
            return langui.get("NoSabe", "No he podido encontrar una respuesta."), debug_info

        # Capa 1: Reglas
        resultado_reglas, debug_reglas = self.buscar_coincidencia_reglas(consulta)
        debug_info["rules"] = debug_reglas
        ...
        if resultado_reglas:
            ...
            return respuesta or langui.get("NoSabe", "Respuesta para la regla no encontrada."), debug_info
        
        # Capa 2: IA
        respuesta_ia, debug_ia = self.ai_responder.answer_question(consulta)
        debug_info["ai"] = debug_ia
        ...
        if respuesta_ia:
            return respuesta_ia, debug_info
...
async def consulta(instance_id: int, data: dict):
    ...
    respuesta, debug_info = validador.procesar_consulta(consulta_texto)
    return JSONResponse({"respuesta": respuesta, "debug_info": debug_info})
...
async def guardar_configuracion(instance_id: int, data: dict):
    instance_path = os.path.join("Instancias", str(instance_id))
    # --- INICIO DE LA CORRECCIÓN ---
    # Asegurarse de que el directorio de la instancia exista antes de cualquier operación
    os.makedirs(instance_path, exist_ok=True)
    # --- FIN DE LA CORRECCIÓN ---
    ...
    try:
        if "respuestas" in data:
            ...
            # --- INICIO DE LA CORRECCIÓN ---
            # Cargar datos solo si el archivo ya existe
            if os.path.exists(respuestas_path):
                with open(respuestas_path, 'r', encoding='utf-8') as f:
                    try:
                        respuestas_data = json.load(f)
                    except json.JSONDecodeError:
                        # Si el archivo está corrupto o vacío, se empieza de cero
                        pass 
            # --- FIN DE LA CORRECCIÓN ---
            
            if "Respuestas" not in respuestas_data or not isinstance(respuestas_data.get("Respuestas"), dict):
                respuestas_data["Respuestas"] = {}
